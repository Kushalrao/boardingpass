...
exports.detectFlightEmail = detectFlightEmail;
const flightDetector_data_1 = require("./flightDetector.data");
// ============================================
// Domain extraction
// ============================================
/**
 ...
 */
function detectFlightEmail(email) {
    let score = 0;
    ...
    let senderCategory = 'unknown';
    if (matchesDomainSet(senderDomain, flightDetector_data_1.AIRLINE_DOMAINS)) {
        senderCategory = 'airline';
        score += 4;
        matchedSignals.push(`airline_sender:${senderDomain}`);
    }
    else if (matchesDomainSet(senderDomain, flightDetector_data_1.OTA_DOMAINS)) {
        senderCategory = 'ota';
        score += 2;
        matchedSignals.push(`ota_sender:${senderDomain}`);
    }
    // --- Step 2: Subject patterns ---
    for (const pattern of flightDetector_data_1.SUBJECT_PATTERNS) {
        if (pattern.regex.test(email.subject)) {
            score += pattern.weight;
            matchedSignals.push(`subject:${pattern.name}`);
            hasNonSenderSignal = true;
            if (pattern.flightSpecific)
    ...
    if (senderCategory !== 'unknown') {
        for (const anti of flightDetector_data_1.ANTI_PATTERNS) {
    ...
    if (!subjectAntiPatternFired && email.body && email.body.length > 20) {
        ...
        for (const keyword of flightDetector_data_1.BODY_KEYWORDS) {
            if (bodyLower.includes(keyword.phrase)) {
                score += keyword.weight;
                matchedSignals.push(`body:${keyword.phrase}`);
                hasNonSenderSignal = true;
                if (keyword.flightSpecific)
                    hasFlightSpecificSignal = true;
            }
        }
        // Airport route pattern (e.g., DEL -> BOM)
        if (flightDetector_data_1.AIRPORT_ROUTE_REGEX.test(email.body)) {
            score += 2;
...
//# sourceMappingURL=flightDetector.js.map