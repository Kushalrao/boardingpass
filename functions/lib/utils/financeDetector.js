...
exports.detectFinanceEmailType = detectFinanceEmailType;
exports.isFinancialEmail = isFinancialEmail;
// ============================================
...
const CC_ISSUER_DOMAINS = new Set([
...
const ALL_FINANCIAL_DOMAINS = new Set([
    ...BANK_DOMAINS,
    ...UPI_DOMAINS,
    ...CC_ISSUER_DOMAINS,
    ...INVESTMENT_DOMAINS,
...
// Keyword Patterns
// ============================================
const OTP_AUTH_PATTERNS = /\b(OTP|one\s*time\s*password|verification\s*code|secure\s*code|login\s*alert|password\s*reset|2FA)\b/i;
const STATEMENT_PATTERNS = /\b(e-?statement|account\s*statement|statement)\b/i;
const CC_IN_SUBJECT_PATTERNS = /\b(credit\s*card|card\s*statement)\b/i;
const SALARY_PATTERNS = /\b(salary|payroll)\b|credited.*salary|salary.*credited/i;
const EMI_PATTERNS = /\b(EMI|equated\s*monthly|loan)\b/i;
const REFUND_PATTERNS = /\b(refund|reversal|cashback\s*credited)\b/i;
const INVESTMENT_PATTERNS = /\b(SIP|mutual\s*fund|units\s*allotted|NAV|dividend)\b/i;
const UPI_PATTERNS = /\b(UPI|unified\s*payment)\b/i;
const CC_TRANSACTION_PATTERNS = /\b(card|credit\s*card)\b.*\b(debited|used|spent|charged)\b|\b(debited|used|spent|charged)\b.*\b(card|credit\s*card)\b/i;
const GENERAL_TRANSACTION_PATTERNS = /\b(debited|credited|transaction|transferred|received|withdrawn)\b/i;
// ============================================
 ...
 */
function isGooglePaySender(from) {
    const bracketMatch = from.match(/<([^>]+)>/);
    const email = bracketMatch ? bracketMatch[1] : from.trim();
...
// ============================================
function classifyBySubject(subject, domain) {
 ...
 *  4. Default: 'unknown'
 */
function detectFinanceEmailType(subject, from) {
    // Normalize inputs
    const sub = subject || '';
    const sender = from || '';
