...
// ============================================
const KNOWN_BANK_EMAIL_DOMAINS = new Set([
    // === PUBLIC SECTOR BANKS ===
    'sbi.co.in',
    'pnb.co.in',
    'bankofbaroda.co.in',
 ...
 */
function isLikelyAadhaar(match, fullText, offset) {
    ...
    if (/(?:Rs\.?|INR|₹)\s*$/i.test(beforeMatch))
        return false;
    return true;
}
/**
 ...
 */
function isPrecededByCurrency(text, offset) {
 ...
 */
function isPrecededByReference(text, offset) {
...
function redactPII(content) {
    ...
    result = result.replace(CREDIT_CARD_PATTERN, (_match, digits) => {
    ...
    // ---- 8. CVV/CVC ----
    result = result.replace(CVV_PATTERN, (match, cvvDigits) => {
...
function redactPhoneNumbers(text) {
    ...
    let result = text.replace(PHONE_WITH_PREFIX_PATTERN, '[REDACTED_PHONE]');
    ...
    result = result.replace(PHONE_STANDALONE_PATTERN, (match, _digits, offset) => {
        const fullText = result;
        // Don't redact if preceded by currency (it's an amount)
        if (isPrecededByCurrency(fullText, offset))
            return match;
        // Don't redact if preceded by reference labels
        if (isPrecededByReference(fullText, offset))
            return match;
