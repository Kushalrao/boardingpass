...
exports.BANK_PASSWORD_HINTS = void 0;
exports.generatePasswordCandidates = generatePasswordCandidates;
/**
 ...
 */
function generatePasswordCandidates(_provider, senderDomain, dob, userName, maskedNumber) {
    ...
    const candidates = [];
    const seen = new Set();
    function add(password, source) {
        if (!password || password.length < 4 || seen.has(password))
            return;
        seen.add(password);
        ...
        for (const pattern of bankPatterns) {
            switch (pattern) {
                ...
                case 'NAME4_UPPER+LAST4':
                    if (name4Upper && last4)
                        add(`${name4Upper}${last4}`, 'NAME4(UPPER)+last4card');
                    break;
                case 'NAME4_LOWER+LAST4':
                    if (name4Lower && last4)
                        add(`${name4Lower}${last4}`, 'name4(lower)+last4acct');
                    break;
                // NAME3 + DOB
                case 'DDMM+NAME3_UPPER':
                    if (name3Upper)
                        add(`${dd}${mm}${name3Upper}`, 'DDMM+NAME3(UPPER)');
                    break;
                ...
                case 'DDMMYYYY+LAST4':
                    if (last4)
                        add(`${dd}${mm}${yyyy}${last4}`, 'DDMMYYYY+last4card');
                    break;
                case 'DDMMYY+LAST4':
                    if (last4)
                        add(`${dd}${mm}${yy}${last4}`, 'DDMMYY+last4card');
                    break;
                default: break;
            }
        }
...
//# sourceMappingURL=statementPasswordPatterns.js.map