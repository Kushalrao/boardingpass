...
function getClassificationPrompt(currentDate: string): string {
...
export class AIChatService {
  ...
  private async classifyQuestion(
    ...
    if (!textBlock || textBlock.type !== 'text') {
      throw new functions.https.HttpsError(
        'internal',
        'Failed to classify question: no response from AI'
      );
    }

    try {
      const parsed = extractJson(textBlock.text) as QuestionClassification;
      ...
      switch (need) {
        ...
        case 'recurring_patterns': {
          const snap = await this.db
            ...
            .get();
          if (snap.exists) {
            const data = snap.data() as RecurringAggregate;
  ...
  private async generateAnswer(
    ...
    if (!textBlock || textBlock.type !== 'text') {
      throw new functions.https.HttpsError(
        'internal',
        'Failed to generate answer: no response from AI'
      );
    }
  ...
  private async checkRateLimit(userId: string): Promise<void> {
    ...
    await this.db.runTransaction(async (txn) => {
      ...
      let hourlyCount: number = data.hourlyCount || 0;
      let hourlyResetAt: number = data.hourlyResetAt || 0;
      let dailyCount: number = data.dailyCount || 0;
      let dailyResetAt: number = data.dailyResetAt || 0;
      ...
      if (hourlyCount >= 30) {
        ...
        throw new functions.https.HttpsError(
          'resource-exhausted',
          `You've reached the limit of 30 questions per hour. Please try again in ${minutesLeft} minute${minutesLeft === 1 ? '' : 's'}.`
        );
      }
      ...
      if (dailyCount >= 200) {
        throw new functions.https.HttpsError(
          'resource-exhausted',
          'You\'ve reached the limit of 200 questions per day. Please try again tomorrow.'
        );
      }
  ...
  private async saveMessages(
    ...
    const snap = await sessionRef.get();
    const session = snap.exists ? (snap.data() as ChatSession) : null;
   ...
   */
  private formatMonthlyAggregate(data: FirebaseFirestore.DocumentData, period: string): string {
    const agg = data as MonthlyAggregate;
    const lines: string[] = [];
    ...
    if (agg.categoryBreakdown && Object.keys(agg.categoryBreakdown).length > 0) {
      ...
      for (const [category, info] of sorted) {
        lines.push(
          `  ${rank}. ${this.capitalizeCategory(category)}: ${formatINR(info.total)} (${info.count} transaction${info.count === 1 ? '' : 's'}, avg ${formatINR(info.avgTransaction)})`
        );
        rank++;
      }
    }
    ...
    if (agg.merchantBreakdown && Object.keys(agg.merchantBreakdown).length > 0) {
      ...
      for (const [merchant, info] of sorted) {
        lines.push(
          `  ${rank}. ${merchant}: ${formatINR(info.total)} (${info.count} transaction${info.count === 1 ? '' : 's'})`
        );
        rank++;
      }
    }
    ...
    if (agg.largestTransaction) {
      lines.push('');
      lines.push(
        `Largest Transaction: ${formatINR(agg.largestTransaction.amount)} at ${agg.largestTransaction.merchant} on ${agg.largestTransaction.date}`
      );
    }
    if (agg.smallestTransaction) {
      lines.push(
        `Smallest Transaction: ${formatINR(agg.smallestTransaction.amount)} at ${agg.smallestTransaction.merchant} on ${agg.smallestTransaction.date}`
      );
    }
   ...
   */
  private formatYearlyAggregate(data: FirebaseFirestore.DocumentData, year: string): string {
    const agg = data as YearlyAggregate;
    const lines: string[] = [];
    ...
    if (agg.categoryBreakdown && Object.keys(agg.categoryBreakdown).length > 0) {
      ...
      for (const [category, info] of sorted) {
        lines.push(
          `  ${rank}. ${this.capitalizeCategory(category)}: ${formatINR(info.total)} (${info.count} transaction${info.count === 1 ? '' : 's'})`
        );
        rank++;
      }
    }
    ...
    if (agg.monthlyTrend && agg.monthlyTrend.length > 0) {
      ...
      for (const entry of agg.monthlyTrend) {
        lines.push(
          `  ${this.formatMonthLabel(entry.month)}: Spending ${formatINR(entry.spending)}, Income ${formatINR(entry.income)}`
        );
      }
    }
   ...
   */
  private formatTransactions(transactions: Transaction[], label: string): string {
    ...
    for (const txn of transactions) {
      ...
      lines.push(
        `  ${rank}. ${dateStr} - ${txn.merchant} - ${typeIndicator}${formatINR(txn.amount)} (${this.capitalizeCategory(txn.category)})${accountLabel ? ' - ' + accountLabel : ''}`
      );
      rank++;
    }
   ...
   */
  private formatRecurringPatterns(data: RecurringAggregate): string {
    ...
    for (const pattern of data.patterns) {
      lines.push(
        `  ${rank}. ${pattern.merchant}: ${formatINR(pattern.amount)} (${pattern.frequency}) - Category: ${this.capitalizeCategory(pattern.category)}, Next expected: ${pattern.nextExpected}`
      );
      rank++;
    }
   ...
   */
  private formatTransactionDate(date: FirebaseFirestore.Timestamp): string {
    const shortMonths = [
