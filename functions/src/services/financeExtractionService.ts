...
export class FinanceExtractionService {
  ...
  async extractTransaction(
    ...
    try {
      ...
      return parsed as unknown as ExtractedTransaction;
    } catch (error) {
   ...
   */
  async extractStatement(content: string): Promise<ExtractedStatement | null> {
    try {
      const response = await this.client.messages.create({
      ...
      const parsed = extractJson(textBlock.text) as ExtractedStatement;
   ...
   */
  async extractFromPdf(pdfBuffer: Buffer): Promise<ExtractedStatement | null> {
    try {
