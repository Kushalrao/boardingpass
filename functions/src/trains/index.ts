...
import { getTrainScraperService } from './services/trainScraperService';
import {
  PnrStatus,
  TrainSchedule,
  LiveStatus,
 ...
 */
export const getTrainPnrStatus = functions
  .runWith({
    timeoutSeconds: 120,
    memory: '512MB',
  })
  .https.onCall(async (data: { pnr: string }): Promise<PnrStatus> => {
    const { pnr } = data;

    if (!pnr || !/^\d{10}$/.test(pnr)) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'Invalid PNR. Must be 10 digits.'
      );
    }

    console.log(`Fetching PNR status for: ${pnr}`);

    const scraperService = getTrainScraperService();
 ...
 * Public function - no authentication required
 */
export const getTrainSchedule = functions
  .runWith({
    timeoutSeconds: 120,
    memory: '512MB',
  })
  .https.onCall(async (data: { trainNumber: string }): Promise<TrainSchedule> => {
    const { trainNumber } = data;

    if (!trainNumber || !/^\d{5}$/.test(trainNumber)) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'Invalid train number. Must be 5 digits.'
      );
    }
 ...
 * Public function - no authentication required
 */
export const getTrainLiveStatus = functions
  .runWith({
    timeoutSeconds: 120,
    memory: '512MB',
  })
  .https.onCall(async (data: { trainNumber: string; date?: string }): Promise<LiveStatus> => {
 ...
 * Public function - no authentication required
 */
export const searchTrains = functions
  .runWith({
    timeoutSeconds: 60,
    memory: '256MB',
  })
  .https.onCall(async (data: { from: string; to: string; date?: string }): Promise<SearchTrainsResult> => {
    const { from, to, date } = data;

    if (!from || !to) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'Both from and to station codes are required.'
      );
    }
 ...
 */
export const addTrainBooking = functions
  .runWith({
    timeoutSeconds: 120,
    memory: '512MB',
  })
  .https.onCall(async (data: AddTrainRequest, context): Promise<{ success: boolean; trainId: string; data: TrainBooking }> => {
    ...
    if (!pnr && !trainNumber) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'Either PNR or train number is required.'
      );
    }
    ...
    if (pnr) {
      ...
      if (!pnrStatus.success) {
        throw new functions.https.HttpsError(
          'not-found',
          pnrStatus.error || 'Could not fetch PNR status'
        );
      }
      ...
      if (!fromStation || !toStation) {
        throw new functions.https.HttpsError(
          'invalid-argument',
          'From and to stations are required when not using PNR.'
        );
      }
 ...
 */
export const getUserTrains = functions
  .runWith({
    timeoutSeconds: 30,
    memory: '256MB',
  })
  .https.onCall(async (data: GetUserTrainsRequest, context): Promise<{ trains: TrainBooking[] }> => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        'unauthenticated',
        'User must be authenticated'
 ...
 * Requires authentication
 */
export const refreshTrainStatus = functions
  .runWith({
    timeoutSeconds: 120,
    memory: '512MB',
  })
  .https.onCall(async (data: RefreshTrainRequest, context): Promise<{ success: boolean; data: TrainBooking }> => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        'unauthenticated',
        'User must be authenticated'
    ...
    const trainData = trainDoc.data() as TrainBooking;
    const scraperService = getTrainScraperService();
    ...
    if (trainData.pnr) {
      ...
      if (pnrStatus.success) {
        const updates: Partial<TrainBooking> = {
          passengers: pnrStatus.passengers,
          chartStatus: pnrStatus.chartStatus,
          overallStatus: pnrStatus.overallStatus,
          confirmationChance: pnrStatus.confirmationChance,
 ...
 * Requires authentication
 */
export const deleteTrainBooking = functions
  .runWith({
    timeoutSeconds: 30,
    memory: '256MB',
  })
  .https.onCall(async (data: { trainId: string }, context): Promise<{ success: boolean }> => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        'unauthenticated',
        'User must be authenticated'
 ...
 */
export const onTrainCreated = functions.firestore
  .document('users/{userId}/trains/{trainId}')
  .onCreate(async (snapshot, context) => {
    const { userId } = context.params;
    const trainData = snapshot.data() as TrainBooking;
